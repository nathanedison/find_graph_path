...
# builds node array from input file
def make_array(path):
    arr = []
    with open(path) as file:
        for line in file:
            ...
            except ValueError as e:     # error if input file contains non-numeric values
                return e
    return arr

# builds set containing all possible segments, defined as order-specific node pairings
# validates data in input file
def build_seg_set(node_links):
    seg_set = set()
            ...
            for neighbor in node_links[n]:
                try:
                    if not n in node_links[neighbor]:
                        raise ValueError('Inconsistent values in node array. Each child node must mutally reference its parent node.')
                except Exception as e:  # In addition to above ValueError, there will be an IndexError if any "neighbor" refers to a non-existant node
                    return e
                seg = (n,neighbor)
...
# This is the core function that traverses the network of nodes to find paths matching the specifications
def find_path(current_node,last_node,path_segs,leg_length,start_nodes):
    if leg_length == 0:     # number of segments from last end node
    ...
    for neighbor in node_links[current_node]:
        ...
        if not seg in path_segs:    # prevents duplicate segments
            # set variables for recurring instance of find_path
            new_node = neighbor
            new_last_node = current_node
            new_path = path_segs + [seg]
            new_leg_length = leg_length + 1
            if new_node in end_nodes:
                if new_leg_length < min_leg:       # prevents path to end node with leg length less than minimum
                    ...
                    else:                       # path reaches last endpoint without including all segments
                        continue
            find_path(
...
# parser to handle command-line argument intake
...
# generate and validate data structures
...
if len(end_nodes) == 0:
    ...
    sys.exit()
seg_set = build_seg_set(node_links)
